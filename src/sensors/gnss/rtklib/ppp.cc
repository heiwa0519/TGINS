...
#define VAR_BIAS    SQR(30.0)       /* init variance phase-bias (m^2) */
...
extern int pppoutstat(rtk_t *rtk, char *buff)
{
    ...
    /* receiver clocks */
    i=tc?xiClk(rtk->fusing->opts):IC(0,&rtk->opt);
    ...
    if (rtk->opt.tropopt==TROPOPT_EST||rtk->opt.tropopt==TROPOPT_ESTG) {
        i=tc?xiTrp(rtk->fusing->opts):IT(&rtk->opt);
    ...
    if (rtk->opt.ionoopt==IONOOPT_EST) {
        for (i=0;i<MAXSAT;i++) {
            ssat=rtk->ssat+i;
            if (!ssat->vs) continue;
            j=tc? xiIon(rtk->fusing->opts,i+1):II(i+1,&rtk->opt);
...
static void udclk_ppp(rtk_t *rtk)
{
    ...
    for (i=0;i<NSYS;i++) {
        ...
        iclk=rtk->tc?xiClk(rtk->fusing->opts)+i:IC(i,&rtk->opt);
        initx(rtk,CLIGHT*dtr,VAR_CLK,iclk);
    }
}
...
static void udtrop_ppp(rtk_t *rtk)
{
    ...
    itrp=rtk->tc?xiTrp(rtk->fusing->opts):IT(&rtk->opt);
    trace(3,"udtrop_ppp:\n");
    
    if (rtk->x[itrp]==0.0) {
        ecef2pos(rtk->sol.rr,pos);
        ztd=sbstropcorr(rtk->sol.time,pos,azel,&var);
        initx(rtk,ztd,var,itrp);
        
        if (rtk->opt.tropopt>=TROPOPT_ESTG) {
            for (j=itrp+1;j<itrp+3;j++) initx(rtk,1E-6,VAR_GRA,j);
        }
    }
    else {
        rtk->P[itrp+itrp*rtk->nx]+=SQR(rtk->opt.prn[2])*fabs(rtk->tt);
        
        if (rtk->opt.tropopt>=TROPOPT_ESTG) {
            for (j=itrp+1;j<itrp+3;j++) {
...
static void udiono_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n;i++) {
        j=rtk->tc?xiIon(rtk->fusing->opts,obs[i].sat):II(obs[i].sat,&rtk->opt);
        if (rtk->x[j]==0.0) {
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    int i,j,k,f,sat,slip[MAXOBS]={0},clk_jump=0,iamb;
    ...
    for (f=0;f<NF(&rtk->opt);f++) {
        ...
        for (i=k=0;i<n&&i<MAXOBS;i++) {
            sat=obs[i].sat;
            j=rtk->tc?xiAmb(rtk->fusing->opts,sat,f):IB(sat,f,&rtk->opt);
        ...
        if (k>=2&&fabs(offset/k)>0.0005*CLIGHT) {
            for (i=0;i<MAXSAT;i++) {
                j=rtk->tc?xiAmb(rtk->fusing->opts,i+1,f):IB(i+1,f,&rtk->opt);
        ...
        for (i=0;i<n&&i<MAXOBS;i++) {
            ...
            iamb=rtk->tc?xiAmb(rtk->fusing->opts,sat,f):IB(sat,f,&rtk->opt);
            initx(rtk,bias[i],VAR_BIAS,iamb);
...
static int model_trop(gtime_t time, const double *pos, const double *azel,
                      const prcopt_t *opt, const double *x,int itrp, double *dtdx,
                      const nav_t *nav, double *dtrp, double *var)
{
    ...
    if (opt->tropopt==TROPOPT_EST||opt->tropopt==TROPOPT_ESTG) {
        matcpy(trp,x+itrp,opt->tropopt==TROPOPT_EST?1:3,1);
...
static int ppp_res(int post, const obsd_t *obs, int n, const double *rs,
                   ...
                   const double *re,const double *dr, int *exc, const nav_t *nav,
                   const double *x, rtk_t *rtk, double *v, double *H, double *R,
                   double *azel)
{
    prcopt_t *opt=&rtk->opt;
    double y,r,cdtr,bias,C=0.0,pos[3],e[3],dtdx[3],L[NFREQ],P[NFREQ],Lc,Pc;
    double var[MAXOBS*2],dtrp=0.0,dion=0.0,vart=0.0,vari=0.0,dcb,freq;
    double dantr[NFREQ]={0},dants[NFREQ]={0},rr[3];
    ...
    int iclk,itrp,iion,ifcb,iamb,tc=rtk->tc;

    time2str(obs[0].time,str,2);
    
    for (i=0;i<MAXSAT;i++) for (j=0;j<opt->nf;j++) rtk->ssat[i].vsat[j]=0;

    for(i=0;i<3;i++) rr[i]=re[i]+dr[i];
    ecef2pos(rr,pos);
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        itrp=tc?xiTrp(rtk->fusing->opts):IT(opt);
        if (!model_trop(obs[i].time,pos,azel+i*2,opt,x,itrp,dtdx,nav,&dtrp,&vart)||
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            if(rtk->tc){
                ...
                if(rtk->fusing->opts.imu.nav_coord==+E_InsNavCoord::ECEF){
                    for(k=xiP();k<xiP()+xnP();k++){
                        H[k+nx*nv]=e[k-xiP()];  /* translation of innovation to position states */
                    }
                }
                else{
                    Vector3d mu;
                    for(k=0;k<3;k++) mu[k]=e[k];
                    mu=mu.transpose()*rtk->fusing->ins.earth.DPne;
                    for(k=xiP();k<xiP()+xnP();k++){
                        H[k+nx*nv]=mu[k-xiP()];                     /* translation of innovation to position states */
                    }
                }
            }
            else{
            ...
            iclk=tc?xiClk(rtk->fusing->opts)+k:IC(k,opt);
            cdtr=x[iclk];
            H[iclk+nx*nv]=1.0;
            ...
            if (opt->ionoopt==IONOOPT_EST) {
                iion=tc?xiIon(rtk->fusing->opts,sat):II(sat,opt);
                if (rtk->x[iion]==0.0) continue;
                H[iion+nx*nv]=C;
            }
            if (frq==2&&code==1) { /* L5-receiver-dcb */
                ifcb=tc?xiIfcb(rtk->fusing->opts):ID(opt);
                dcb+=rtk->x[ifcb];
                H[ifcb+nx*nv]=1.0;
            }
            if (code==0) { /* phase bias */
                iamb=tc?xiAmb(rtk->fusing->opts,sat,frq):IB(sat,frq,opt);
...
static void update_stat(rtk_t *rtk, const obsd_t *obs, int n, int stat, ins_t *ins)
{
    const prcopt_t *opt=&rtk->opt;
    int i,j,tc=rtk->tc,iclk;
    ...
    if (rtk->sol.stat==SOLQ_FIX) {
        if(rtk->tc){
            getStateCov(rtk->fusing->ep_kf.Pa,ins->state);
            rtk->fusing->ig_ins=*ins;
        }else{
            for (i=0;i<3;i++) {
    ...
    iclk=tc? xiClk(rtk->fusing->opts):IC(0,opt);
    rtk->sol.dtr[0]=rtk->x[iclk]; /* GPS */
    rtk->sol.dtr[1]=rtk->x[iclk+1]-rtk->x[iclk]; /* GLO-GPS */
    rtk->sol.dtr[2]=rtk->x[iclk+2]-rtk->x[iclk]; /* GAL-GPS */
    rtk->sol.dtr[3]=rtk->x[iclk+3]-rtk->x[iclk]; /* BDS-GPS */
...
{
    const prcopt_t *opt=&rtk->opt;
    double *rs,*dts,*var,*v,*H,*R,*azel,*xp,*Pp,dr[3]={0},std[3],re[3];
    char str[32];
    ...
    for (i=0;i<MAX_ITER;i++) {
        ...
        else matcpy(re,xp,3,1);

        /* prefit residuals */
        if (!(nv=ppp_res(0,obs,n,rs,dts,var,svh, re,dr,exc,nav,xp,rtk,v,H,R,azel))) {
        ...
        /* postfit residuals */
        if (ppp_res(i+1,obs,n,rs,dts,var,svh, re,dr,exc,nav,xp,rtk,v,H,R,azel)) {
            matcpy(rtk->x,xp,rtk->nx,1);
            matcpy(rtk->P,Pp,rtk->nx,rtk->nx);
            if(rtk->tc){
                ins_float.gstat=SOLQ_PPP;
    ...
    if (stat==SOLQ_PPP) {
        if (ppp_ar(rtk,obs,n,exc,nav,azel,xp,Pp)) {
            if(rtk->tc){
                ins_fix=ins_float;
                insFeedback(rtk->fusing->opts.imu,rtk->fusing->opts.imu.nav_coord, rtk->fusing->opts.imu.att_def, ins_fix,xp,rtk->nx);
            ...
            if(ppp_res(9,obs,n,rs,dts,var,svh, re,dr,exc,nav,xp,rtk,v,H,R,azel)){
        ...
        /* update solution status */
        update_stat(rtk,obs,n,stat,stat==SOLQ_FIX?&ins_fix:&ins_float);
