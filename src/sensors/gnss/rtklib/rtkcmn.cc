...
const solopt_t solopt_default={ /* defaults solution output options */
    ...
    0,1,0,0,0,0,0,0,              /* degf,outhead,outopt,outvel,datum,height,geoid */
                  ...
                  const double *R, int n, int m,int tra)
{
    ...
    if(tra==1||tra==3){
        matprint(1,x_,k,1,20,10,"Prior x:");
        matprint(1,P_,k,k,15,6,"Prior P:");
        matprint(1,v,m,1,15,6,"Innovation:");
        matprint(1,H_,k,m,15,6,"H:");
        matprint(1,R,m,m,15,6,"R:");
    }
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_);

    if(tra==2||tra==3){
        matprint(1,xp_,k,1,20,10,"Post x:");
        matprint(1,Pp_,k,k,15,6,"Post P:");
    }
...
extern double  time2gpst_new(gtime_t t, int *week,int *dow)
{
    gtime_t t0=epoch2time(gpst0);
    time_t sec=t.time-t0.time;
    int w=(int)(sec/(86400*7));
    ...
    if (week) *week=w;
    if (dow)  *dow=(int)((sec-w*86400*7)/86400);
    return (double)(sec-(double)w*86400*7)+t.sec;
}
...
extern gtime_t yrdoy2time(int year,int doy)
{
    double ep[6]={ 0 };
    ep[0] = year; ep[1] = 1; ep[2] = 1;
    ep[3] = 0;    ep[4] = 0; ep[5] = 0;
    gtime_t t_jan1 = epoch2time(ep);
...
extern double time2doy_new(gtime_t t,int *year)
{
    double ep[6];
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    char dir[1024]="",s1[1024],s2[1024],*p,*q,*r,path_[1024];
