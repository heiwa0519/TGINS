...
static int tonum(char *buff, const char *sep, double *v)
{
    ...
    for (p=bu,n=0;n<MAXFIELD;p=q+len) {
...
static int decode_solins(char *buff,int start,const solopt_t *opt,sol_t *sol){
    double val[MAXFIELD];
    int i=0,j,n;
    const char *sep=opt2sep(opt);

    trace(4,"decode_solins:\n");
    ...
    if(i+3<n){
        sol->ba[0]=val[j++]*MG2MPS2;
        sol->ba[1]=val[j++]*MG2MPS2;
        sol->ba[2]=val[j++]*MG2MPS2;
    }

    if(i+3<n){
        sol->bg[0]=val[j++]*DPH2RPS;
        sol->bg[1]=val[j++]*DPH2RPS;
        sol->bg[2]=val[j++]*DPH2RPS;
    }

    if(i+3<n){
        sol->sg[0]=val[j++]*PPM2SCALE;
        sol->sg[1]=val[j++]*PPM2SCALE;
        sol->sg[2]=val[j++]*PPM2SCALE;
    }

    if(i+3<n){
        sol->sa[0]=val[j++]*PPM2SCALE;
        sol->sa[1]=val[j++]*PPM2SCALE;
        sol->sa[2]=val[j++]*PPM2SCALE;
    }
...
static int decode_solstat(char *buff, solstat_t *stat)
{
    ...
    double tow,az,el,resp,resc,res_tdcp,res_dop,td_dif,snr;
    ...
    n=sscanf(buff,"$SAT%d%lf%s%d%lf%lf%lf%lf%lf%lf%lf%d%lf%d%d%d%d%d",
             &week,&tow,id,&frq,&az,&el,&resp,&resc,&res_tdcp,&res_dop,&td_dif,&vsat,&snr,&fix,&slip,
    ...
    *stat=stat0;
    ...
    stat->res_tdcp =(float)res_tdcp;
    stat->res_dop =(float)res_dop;
    stat->td_dif  =(float)td_dif;
                   ...
                   const solopt_t *opt)
{
    ...
    if(norm(sol->rpy,3)!=0.0){
        p+= outInsState(reinterpret_cast<uint8_t *>(p), s, sol, opt);
    }
...
static int outenu(uint8_t *buff, const char *s, const sol_t *sol,
                  const double *rb, const solopt_t *opt,int err_fmt)
{
    double pos[3],rr[3],enu[3],P[9],Q[9],vn[3];
    int i;
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
    ...
    if(err_fmt){
        matcpy(enu,sol->rr,1,3);
        matcpy(vn,sol->rr+3,1,3);
    }
    else{
    ...
    if (opt->outvel) { /* output velocity */
        if(err_fmt){
            ...
            soltocov_vel(sol,P);
            ecef2enu(pos,sol->rr+3,vn);
            covenu(pos,P,Q);
        }

        p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s"
                     "%8.5f%s%8.5f",
                   sep,vn[1],sep,vn[0],sep,vn[2],sep,SQRT(Q[4]),sep,
                   SQRT(Q[0]),sep,SQRT(Q[8]),sep,sqvar(Q[1]),sep,sqvar(Q[2]),
                   sep,sqvar(Q[5]));
    }
...
extern int outprcopts(uint8_t *buff, const prcopt_t *opt)
{
    ...
    const char *s1[]={
        "Sol","Single","DGPS","Kinematic","Static","Static-Start","Moving-Base","Fixed",
    ...
    p+=sprintf(p,"%s pos mode  : %s\r\n",COMMENTH,s1[opt->mode==-1?0:opt->mode]);
...
extern int outsolheads(uint8_t *buff, const solopt_t *opt)
{
    ...
    p+=sprintf(p,"%s%3s%s%10s%s%10s%s%10s%s%9s%s%8s%s%8s%s%8s%s%8s%s%8s",
               sep,"gs",sep,"roll(deg)",sep,"pitch(deg)",sep,"yaw(deg)",sep,"sdr",sep,
               "sdp",sep,"sdy",sep,"sdrp",sep,"sdpy",sep,"sdyr");
    p+= sprintf(p,"%s%8s%s%8s%s%8s",sep,"bax(mg)",sep,"bay(mg)",sep,"baz(mg)");

    p+= sprintf(p,"%s%10s%s%10s%s%10s",sep,"bgx(deg/h)",sep,"bgy(deg/h)",sep,"bgz(deg/h)");

    p+= sprintf(p,"%s%8s%s%8s%s%8s",sep,"sgx(ppm)",sep,"sgy(ppm)",sep,"sgz(ppm)");

    p+= sprintf(p,"%s%8s%s%8s%s%8s",sep,"sax(ppm)",sep,"say(ppm)",sep,"saz(ppm)");
...
extern int outsols(uint8_t *buff, const sol_t *sol, const double *rb,
                   const solopt_t *opt,int err_fmt)
{
    gtime_t time,ts={0};
    double gpst;
    int week,timeu;
    ...
    switch (opt->posf) {
        ...
        case SOLF_ENU:  p+=outenu(p,s,sol,rb,opt,err_fmt); break;
                   ...
                   const solopt_t *opt,int err_fmt)
{
    ...
    if ((n=outsols(buff,sol,rb,opt,err_fmt))>0) {
        fwrite(buff,n,1,fp);
    }
}
/* output solution extended ----------------------------------------------------
