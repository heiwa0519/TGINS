...
#define VER_RTKLIB  "v0.1"               /* library version */

#define PATCH_LEVEL "p01"                /* patch level */
...
#define MINPRNSBS   0                /* min satellite PRN number of SBAS 120*/
#define MAXPRNSBS   0                /* max satellite PRN number of SBAS 158*/
#define NSATSBS     (MAXPRNSBS-MINPRNSBS+1-1) /* number of SBAS satellites */
...
typedef struct{
    double code[2][NFREQ];     /*prior/post-fit pseudorange residual*/
    double phase[2][NFREQ];    /*prior/post-fit phase residual*/
    double tdcp[2][NFREQ];     /*prior/post-fit tdcp residual*/
    double doppler[2][NFREQ];  /*prior/post-fit doppler residual*/
}measres_t;

typedef struct{
    gtime_t pre_t[2][NFREQ];
    double pre_L[2][NFREQ];
    double pre_rho;
    double val[NFREQ];
}tdcp_t;
...
typedef struct {        /* satellite status type */
    ...
    double res_tdcp[NFREQ];
    double res_dop[NFREQ];
    double td_dif[NFREQ];
...
typedef struct {        /* RTK control/result type */
    ...
    int stc,tc;
    ins_aid_t ins_pred;
    int epoch,fix_epoch,hold_epoch;
} rtk_t;
...
EXPORT int  filter(double *x, double *P, const double *H, const double *v,
                   const double *R, int n, int m,int tra);
...
EXPORT void matprint(int trans,const double A[], int n, int m, int p, int q,const char *remark);
...
EXPORT double  time2gpst_new(gtime_t t, int *week,int *dow);
...
EXPORT gtime_t yrdoy2time(int year,int doy);
...
EXPORT double  time2doy_new(gtime_t t,int *year);
...
/*input gnss obs function*/
extern int nextobsf(const obs_t *obs, int *i, int rcv);
extern int nextobsb(const obs_t *obs, int *i, int rcv);

/* standard positioning ------------------------------------------------------*/
EXPORT double prange(const obsd_t *obs, const nav_t *nav, const prcopt_t *opt,
                     double *var);
EXPORT double varerr(const prcopt_t *opt, const ssat_t *ssat, const obsd_t *obs, double el, int sys);
EXPORT int snrmask(const obsd_t *obs, const double *azel, const prcopt_t *opt);
EXPORT int calTdcp(const obsd_t *obs,int n,ssat_t *ssat);
EXPORT int calRange(const prcopt_t *opt,const obsd_t *obs,int n,const nav_t *nav,const double *rr,ssat_t *ssat);
EXPORT int restdcp(const prcopt_t *opt,const obsd_t *obs, int n, const double *rs, const double *dts,
...
EXPORT void readpreceph(char **infile, int n, const prcopt_t *prcopt,
                        nav_t *nav, sbs_t *sbs);
